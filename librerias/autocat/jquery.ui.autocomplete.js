...
//Se encarga de llenar el campo al pasar el mouse por encima
				...
				selected: function( event, ui ) {
					...
					if ( false !== self._trigger( "select", event, { item: item } ) ) {
						...
						var txt2 = item.value.replace(/(?:<(?:script|style)[^>]*>[\s\S]*?<\/(?:script|style)>|<[!\/]?[a-z]\w*(?:\s*[a-z][\w\-]*=?[^>]*)*>|<!--[\s\S]*?-->|<\?[\s\S]*?\?>)[\r\n]*/gi, '');
						//alert(txt2);
						document.getElementById('foreingkey').value=item.id;
						document.getElementById('modelo_id').value=item.aut_model;
						self.element.val( txt2 );
					}
					self.close( event );
					self.previous = self.element.val();
	...
	_renderItem: function( ul, item) {
		return $( "<li></li>" )
			...
			.append( "<a onclick=\"cargarId('"+item.id+"')\" >" + item.label + "</a>" )
			.appendTo( ul );
	},
