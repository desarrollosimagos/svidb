...
jQuery.fn.rte = function(css_url, media_url) {
    ...
    function enableDesignMode(textarea) {
		...
		css = css + 'function emisor(){\n\t\twindow.parent.contador(\'' + iframe.title + '\'); \n}';
		css = css + 'document.oncontextmenu = function() {\n';
      	css = css + 'return false;\n';
   		css = css + '}\n';
   		css = css + 'function right(e) {\n';
      	css = css + '\t\tvar msg = "Por favor use los botones de copiar y pegar dispuestos en el formulario Principal";\n';
      	css = css + '\t\tif (navigator.appName == "Netscape" && e.which == 3) {\n';
        css = css + '\t\talert(msg);\n';
        css = css + '\t\treturn false;\n';
      	css = css + '}\n';
     	css = css + 'else if (navigator.appName == "Microsoft Internet Explorer" && event.button==2) {\n';
        css = css + '\t\talert(msg);\n';
      	css = css + '\t\treturn false;\n';
      	css = css + '}\n';
   		css = css + '\t\treturn true;\n';
		css = css + '}\n';
		css = css + 'document.onmousedown = right;\n';
		
		css = css + '</script>';
        var doc = "<html><head>"+css+"</head><body class='frameBody' onkeyup=\"emisor();\">"+content+"</body></html>";
    ...
    function toolbar(iframe) {
        ...
        var tb = $("<div class='rte-toolbar' id='toolbar-"+iframe.title+"'><div>\
            <p>\
                <input type='button' class='bold' value='N'> \
                <input type='button' class='italic' value='K'>\
				<input type='button' class='underline' value='U'>\
            </p>\
            <p>\
            </p></div></div>");
        $('.bold', tb).click(function(){ formatText(iframe, 'bold');contador(iframe.title);return false; });
        $('.italic', tb).click(function(){ formatText(iframe, 'italic');contador(iframe.title);return false; });
        $('.underline', tb).click(function(){ formatText(iframe, 'underline');contador(iframe.title);return false; });
