    ...
    if _username :
       ...
       if evento:
          ...
          try:
             ...
             tipo_eventos = None
          if request.method == 'POST':
             superior = request.POST['txtconf']
             coletilla = request.POST['colet']
             modsefl = request.POST['modsefl']
             titulostodos = request.POST['titulostodos']
             ...
             html = render_to_string('documentos/certificadosTodos.html', {'pagesize':'A4 landscape','trabajos':trabajos,'url':d,'MEDIA_ROOT':MEDIA_ROOT,'superior':superior,'coletilla':coletilla,'modsefl':modsefl,'titulostodos':titulostodos}, context_instance=RequestContext(request))
             return generarPDF(html) 
          else:
             return render_to_response("panel/certificado.html",{'evento':evento,'id2':id,'eventoPadre':eventoPadre,'tipo_eventos':tipo_eventos}, context_instance=RequestContext(request))       
       else:
