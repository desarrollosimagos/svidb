...
def buscadorCompleto(request):
    ...
    if request.method == 'GET': 
       ...
       if textos <> "" :
                ...
                try:
                    if persona:
                       if id_usuario.is_staff:
                          ...
                          taxon = Taxon.objects.filter((Q(nombre__icontains=textos) | Q(nombrecomun__icontains=textos) | Q(otrosnombrec__icontains=textos)) & Q(colaboradorespersonas__estatu=0) & Q(colaboradorespersonas__tipoColaboracion=9)).order_by('subtipo__posicion','nombre')
                    else:
                       taxon = Taxon.objects.filter((Q(nombre__icontains=textos) | Q(nombrecomun__icontains=textos) | Q(otrosnombrec__icontains=textos)) & Q(colaboradorespersonas__estatu=0) & Q(colaboradorespersonas__tipoColaboracion=9)).order_by('subtipo__posicion','nombre')
          ...
          return render_to_response('gestion/buscador.html', {'textos':textos,'taxon':taxon,'taxon_id':taxon_id,'subCategoria':subCategoria,'Categoria':Categoria,'etnias':etnias,'banco':banco,'biorregion':biorregion,'eventos':eventos,'mapas':mapas,'actor':actor,'areas':areas,'glosario':glosario,'biblioteca':biblioteca,'modelo':modelo,'datoinicio':datoinicio})   			
       else:
           ...
           if modelo == 'especie':
              if taxon_id <> "":
                 ...
                 try:
                     if persona:
                        if id_usuario.is_staff:
                           taxon = Taxon.objects.filter(Q(nombre__icontains=textos) | Q(nombrecomun__icontains=textos) | Q(otrosnombrec__icontains=textos))
                        else:
                           taxon = Taxon.objects.filter((Q(nombre__icontains=textos) | Q(nombrecomun__icontains=textos) | Q(otrosnombrec__icontains=textos)) & Q(colaboradorespersonas__estatu=0) & Q(colaboradorespersonas__tipoColaboracion=9))
                     else:
                        taxon = Taxon.objects.filter((Q(nombre__icontains=textos) | Q(nombrecomun__icontains=textos) | Q(otrosnombrec__icontains=textos)) & Q(colaboradorespersonas__estatu=0) & Q(colaboradorespersonas__tipoColaboracion=9))
                 except Taxon.DoesNotExist:
                     taxon = None
