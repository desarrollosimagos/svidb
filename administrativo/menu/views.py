...
def listaAreas(request,id):
    ...
    if persona:
       if id_usuario.is_staff:
          ...
          try:
              areas = Areas.objects.filter(Q(subtipoas=principal) & Q(colaboradorespersonas__estatu=0) & (Q(colaboradorespersonas__tipoColaboracion=9) | Q(colaboradorespersonas__tipoColaboracion=18))).order_by('nombre')[:100]
          except Areas.DoesNotExist:
              areas = None
    else:
       try:
...
#           datos = None 
       ...
       if persona:
          if id_usuario.is_staff:
             try:
                 datos = Taxon.objects.filter(Q(taxonrelax__nombre__icontains=x) | Q(nombre__icontains=x) | Q(nombrecomun__icontains=x) | Q(otrosnombrec__icontains=x)).order_by('subtipo','nombre')[:10]
             except Taxon.DoesNotExist:
                 datos = None    
